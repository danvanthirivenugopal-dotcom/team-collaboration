...
def init_api_client():
    ...
    if "BACKEND_URL" in os.environ:
        ...
        try:
            if hasattr(st, "secrets") and st.secrets is not None and "BACKEND_URL" in st.secrets:
                backend_url = st.secrets["BACKEND_URL"]
        except Exception:
            pass
    if "backend" in st.query_params:
        backend_url = st.query_params["backend"]
    return FaceAiApiClient(base_url=backend_url)
...
def reset_auth_session():
    ...
    st.session_state.api = init_api_client()
...
def require_auth():
    is_logged_in = bool(st.session_state.get("authenticated") or st.session_state.get("logged_in") or st.session_state.get("token"))
    if not is_logged_in:
...
def resolve_image_path(filename: str) -> str:
    ...
    for folder in ["users", "admins", "registered", "enrollments"]:
        check_path = UPLOAD_DIR / folder / filename
...
def generate_audit_pdf(audit_logs):
    ...
    class ProfessionalAuditPDF(FPDF):
        def header(self):
            ...
            self.cell(0, 5, "System Audit Logs Report", ln=True, align="C")
            self.ln(12)
        ...
        def footer(self):
            ...
            self.cell(
                0,
                10,
                f"Generated by Face AI Attendance System  |  Page {self.page_no()}",
                align="C"
            )
    ...
    pdf = ProfessionalAuditPDF()
    pdf.set_auto_page_break(auto=True, margin=20)
    pdf.add_page()
    pdf.ln(15)

    total_logs = len(audit_logs)
    unique_admins = len({log.get("admin", "") for log in audit_logs})
    today = datetime.now().strftime("%Y-%m-%d")

    pdf.set_font("helvetica", "B", 12)
    pdf.set_text_color(31, 41, 55)
    pdf.cell(0, 10, f"Generated Date: {today}", ln=True)
    pdf.ln(2)
    ...
    pdf.set_text_color(31, 41, 55)
    pdf.cell(63, 12, f"Total Logs: {total_logs}", border=1, fill=True, align="C")

    pdf.set_fill_color(209, 250, 229)
    pdf.set_text_color(6, 95, 70)
    pdf.cell(63, 12, f"Unique Admins: {unique_admins}", border=1, fill=True, align="C")

    pdf.set_fill_color(219, 234, 254)
    pdf.set_text_color(30, 58, 138)
    pdf.cell(63, 12, "Report Type: Audit Logs", border=1, fill=True, align="C", ln=True)

    pdf.ln(8)

    pdf.set_text_color(31, 41, 55)
    pdf.set_font("helvetica", "B", 12)
    pdf.cell(0, 8, "Detailed Audit Records", ln=True)
    pdf.ln(2)

    col_w = [16, 28, 78, 28, 40]
    headers = ["ID", "Admin", "Action", "Target", "Timestamp"]
    ...
    for idx, log in enumerate(audit_logs, 1):
        ...
        log_id = clean_text(log.get("id", ""))
        admin = clean_text(log.get("admin", ""))
        action = clean_text(log.get("action", ""))
        target = clean_text(log.get("target", ""))
        timestamp = clean_text(log.get("timestamp", ""))
        ...
        for w, value in zip(col_w, row_data):
            pdf.cell(w, 7, value, border=1, fill=fill, align="C")
        pdf.ln()
    ...
    if not pdf_bytes.startswith(b"%PDF"):
        raise ValueError("Generated audit file is not a valid PDF.")
...
def get_cookies() -> dict:
    import urllib.parse
    cookies = {}
    cookie_hdr = st.context.headers.get("Cookie", "")
    if cookie_hdr:
        for cookie in cookie_hdr.split(";"):
            parts = cookie.strip().split("=", 1)
            if len(parts) == 2:
                cookies[parts[0]] = urllib.parse.unquote(parts[1])
    return cookies
...
# Session Restoration from Cookie (survives page reloads)
# Skip cookie restoration if user recently logged out (prevent re-authentication from stale cookies)
if not st.session_state.authenticated and not st.session_state.get("logged_out"):
    cookies = get_cookies()
    token = cookies.get("face_ai_token")
    uid = cookies.get("face_ai_user_id")
    name = cookies.get("face_ai_username")
    role = cookies.get("face_ai_user_role")
    status = cookies.get("face_ai_approval_status")
    
    if token and uid and name and role:
        ...
        st.session_state.user_role = role_map.get(role_normalized, role)
        st.session_state.approval_status = status
        api.set_token(token)

# EARLY TOKEN RESTORE: Ensure API client always has the token set on every Streamlit rerun.
# The API client object may lose its token attribute across reruns; this restores it from session state.
if st.session_state.get("token") and not getattr(api, "token", None):
    try:
        api.set_token(st.session_state.token)
    except Exception:
        pass

# Defer cookie save/clear HTML injection to after page config (flags only)
if "_need_save_cookies" not in st.session_state:
    st.session_state._need_save_cookies = False
if "_need_clear_cookies" not in st.session_state:
    st.session_state._need_clear_cookies = False

# Mark cookies for saving if authenticated
if st.session_state.authenticated:
    st.session_state._need_save_cookies = True

# Mark cookies for clearing if logged out
...
# Deferred cookie HTML injection (now safe - after set_page_config at top)
if st.session_state.get("_need_clear_cookies"):
    st.components.v1.html(
        """
        <script>
        document.cookie = "face_ai_token=; path=/; max-age=0";
        document.cookie = "face_ai_user_id=; path=/; max-age=0";
        document.cookie = "face_ai_username=; path=/; max-age=0";
        document.cookie = "face_ai_user_role=; path=/; max-age=0";
        document.cookie = "face_ai_approval_status=; path=/; max-age=0";
        </script>
        """,
        height=0
    )
    st.session_state._need_clear_cookies = False
    # Keep logged_out flag alive for one more rerun to prevent cookie re-auth
    # It will be cleared on the next navigation action
elif st.session_state.get("_need_save_cookies"):
    st.components.v1.html(
        f"""
        <script>
        const maxAge = 60 * 15; // 15 minutes to match inactivity timeout
        document.cookie = "face_ai_token={api.token or ''}; path=/; max-age=" + maxAge;
        document.cookie = "face_ai_user_id={st.session_state.user_id or ''}; path=/; max-age=" + maxAge;
        document.cookie = "face_ai_username={st.session_state.username or ''}; path=/; max-age=" + maxAge;
        document.cookie = "face_ai_user_role={st.session_state.user_role or ''}; path=/; max-age=" + maxAge;
        document.cookie = "face_ai_approval_status={st.session_state.approval_status or ''}; path=/; max-age=" + maxAge;
        </script>
        """,
        height=0
    )
...
if "fp_reg_result" in st.query_params:
    st.session_state.fp_reg_result = st.query_params["fp_reg_result"]

if "nav" in st.query_params:
    nav_val = st.query_params["nav"]
    valid_nav_values = [
        "Home / Scanner", "Home %2F Scanner", "Feature", "Techstack",
        "Comment", "Team", "Login", "Register", "Profile", "My Account",
        "Logout", "Reports", "Dashboard", "User Dashboard", "User%20Dashboard",
        "Admin Dashboard"
    ]

    if nav_val in valid_nav_values:
        role_key_for_nav = str(st.session_state.get("user_role") or "User").strip().lower().replace(" ", "_").replace("-", "_")

        if nav_val == "Logout":
            # IMPORTANT: clear URL nav first, otherwise ?nav=Logout stays in the URL
            # and Streamlit keeps logging out on every rerun.
            st.query_params.clear()
            logout()
        elif nav_val in ["Home / Scanner", "Home %2F Scanner"]:
            st.session_state.current_page = "Home / Scanner"
        elif nav_val in ["User Dashboard", "User%20Dashboard"]:
            st.session_state.current_page = "User Dashboard"
        elif nav_val == "Dashboard":
            if role_key_for_nav in ["admin", "super_admin", "superadmin", "developer"]:
                st.session_state.current_page = "Admin Dashboard"
            else:
                st.session_state.current_page = "User Dashboard"
        elif nav_val == "Reports":
            st.session_state.current_page = "Reports"
        elif nav_val in ["Profile", "My Account"]:
...
<a href="?nav=Register" class="nav-register-btn" target="_self">Register</a>"""
        ...
        user_role_lower = str(st.session_state.get("user_role") or "User").strip().lower().replace(" ", "_").replace("-", "_")
        if user_role_lower in ["admin", "super_admin", "superadmin", "developer"]:
            report_btn = '<a href="?nav=Reports" class="nav-login-btn" style="margin-right: 0.5rem;" target="_self">Reports</a>'
            
            # In super admin and admin, right side of the report show dashboard button
            if user_role_lower in ["admin", "super_admin", "superadmin"]:
                report_btn += '<a href="?nav=Dashboard" class="nav-login-btn" style="margin-right: 0.5rem;" target="_self">Dashboard</a>'
            
            # In developer, right side of logout show dashboard button
            if user_role_lower == "developer":
                dashboard_after_logout = '<a href="?nav=Dashboard" class="nav-register-btn" style="margin-left: 0.5rem;" target="_self">Dashboard</a>'
                
        elif user_role_lower in ["user", "premium_user", "premiumuser"]:
            # In user, right side of logout show dashboard button
            dashboard_after_logout = '<a href="?nav=User%20Dashboard" class="nav-register-btn" style="margin-left: 0.5rem;" target="_self">Dashboard</a>'
            
        auth_buttons = f"""{report_btn}<a href="?nav=Profile" class="nav-login-btn" target="_self">Profile</a>
<a href="?nav=Logout" class="nav-register-btn" target="_self">Logout</a>{dashboard_after_logout}"""
...
def render_hero():
    if st.session_state.get("authenticated"):
        title = f"Hello, {st.session_state.get('username')}! 👋"
        subtitle = "Welcome to the Face AI Attendance System."
    else:
        title = "Smart Attendance,<br>Powered by AI"
        subtitle = "Real-time face recognition for accurate, secure and contactless attendance tracking."
...
def render_comments_section():
    ...
    try:
        ...
        if not comments_list:
            ...
            for c in comments_list:
                mail_id = c.get("mail_id", "guest")
                user_letter = mail_id[0].upper() if mail_id else "U"
                date_str = c.get("created_at", "")
                text_content = c.get("comment_text", "")
                st.markdown(f"""
                <div class="comment-card">
                    <div class="comment-header">
                        <div class="comment-avatar">{user_letter}</div>
                        <div class="comment-meta">
                            <span class="comment-user">{mail_id}</span>
    ...
    with st.form("comment_post_form", clear_on_submit=True):
        mail_input = st.text_input("Your Email Address", placeholder="Enter your email to post...")
        comment_input = st.text_area("Type your comment here...", placeholder="Enter feedback, bugs, or feature suggestions...")
        post_btn = st.form_submit_button("Post Comment", type="primary")
        if post_btn:
            if not mail_input.strip() or "@" not in mail_input:
                st.warning("Please provide a valid email address.")
            elif not comment_input.strip():
                st.warning("Cannot post an empty comment!")
            else:
                try:
                    # Posting comment with mail_id
                    active_user_id = st.session_state.get("user_id")
                    api.post_comment(active_user_id, mail_input.strip(), comment_input.strip())
...
# render_login_section and render_register_section imported from modules.auth
...
if "pending_checkout_user_id" not in st.session_state:
    st.session_state.pending_checkout_user_id = None
if "pending_checkout_user_name" not in st.session_state:
    st.session_state.pending_checkout_user_name = ""
if "pending_attendance_method" not in st.session_state:
...
# Navigation buttons
# FINAL ROUTING FIX:
# One source of truth: authenticated/logged_in/token all mean logged in.
# Dashboard/Reports/Profile must never reset current_page back to Login after login.
def _is_logged_in() -> bool:
    return bool(
        st.session_state.get("authenticated")
        or st.session_state.get("logged_in")
        or st.session_state.get("token")
        or getattr(api, "token", None)
    )

# Restore API token after every Streamlit rerun.
if st.session_state.get("token") and not getattr(api, "token", None):
    try:
        api.set_token(st.session_state.token)
    except Exception:
...
st.session_state.authenticated = _is_logged_in()
st.session_state.logged_in = _is_logged_in()

def _role_key() -> str:
    role = str(st.session_state.get("user_role") or "User").strip().lower()
    return role.replace(" ", "_").replace("-", "_")

def _go_dashboard():
    role_key = _role_key()

    if role_key in ["admin", "super_admin", "superadmin"]:
        st.session_state.current_page = "Admin Dashboard"
...
if st.session_state.current_page in protected_nav_pages and not _is_logged_in():
    st.session_state.current_page = "Login"
elif st.session_state.current_page not in allowed_pages:
...
if st.session_state.current_page == "Welcome":
    st.markdown(f"""
    <div style="text-align: center; margin-top: 8rem; margin-bottom: 8rem; padding: 3rem 2rem; background: rgba(255, 255, 255, 0.8); border-radius: 16px; box-shadow: 0 4px 30px rgba(0, 0, 0, 0.05); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.3); max-width: 700px; margin-left: auto; margin-right: auto;">
        <h1 style="font-family: 'Outfit', sans-serif; font-size: 2.8rem; color: #1F2937; margin-bottom: 1rem; font-weight: 800;">
            Hello {st.session_state.username},
        </h1>
        <h2 style="font-family: 'Inter', sans-serif; font-size: 1.8rem; color: #4B5563; font-weight: 500; margin: 0;">
            Welcome to Face AI Attendance System
        </h2>
    </div>
    """, unsafe_allow_html=True)
elif st.session_state.current_page == "Feature":
    ...
    render_stats_section()
elif st.session_state.current_page in ["Dashboard", "Manage Users", "Attendance Log"] and st.session_state.current_page != "System Settings":
    # Fixed Dashboard routing: use st.session_state.user_role, not st.session_state.user.
    role = str(st.session_state.get("user_role") or "").strip()

    if st.session_state.current_page == "Manage Users":
        st.session_state.active_part_view = "Total Users"
        go_to_page("Admin Dashboard")

    elif st.session_state.current_page == "Attendance Log":
        st.session_state.active_part_view = "Attendance Log"
        go_to_page("Admin Dashboard")

    elif st.session_state.current_page == "Dashboard":
        role_key_for_dashboard = role.strip().lower().replace(" ", "_").replace("-", "_")
        if role_key_for_dashboard in ["admin", "super_admin", "superadmin"]:
            go_to_page("Admin Dashboard")
        elif role_key_for_dashboard == "premium_user":
            go_to_page("Profile")
        else:
            go_to_page("User Dashboard")
    ...
    # Validate user_id before making API calls
    if not st.session_state.get("user_id") or not isinstance(st.session_state.get("user_id"), int):
    ...
    try:
        ...
        if stats.get("needs_face_update"):
            ...
            if st.button("Update Face Profile Now", type="primary", key="btn_trigger_face_update"):
                ...
                go_to_page("Update Face Profile")
        ...
        # 7 Metric Cards Grid
        col1, col2, col3, col_late = st.columns(4)
        with col1:
            status_val = stats.get('approval_status', 'Pending')
            status_class = "status-approved" if status_val == "Approved" else ("status-pending" if status_val == "Pending" else "status-rejected")
            st.markdown(f"""
            <div class='metric-card'>
        ...
        st.write("---")
        # Show admin/superadmin replies for this user
        try:
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            csv_path = os.path.join(base_dir, "pending_corrections.csv")

            if os.path.exists(csv_path):
                reply_df = pd.read_csv(csv_path)

                if "user_id" in reply_df.columns and "admin_reply" in reply_df.columns:
                    my_replies = reply_df[
                        reply_df["user_id"].astype(str) == str(st.session_state.user_id)
                    ]

                    my_replies = my_replies[
                        my_replies["admin_reply"].fillna("").astype(str).str.strip() != ""
                    ]

                    if not my_replies.empty:
                        st.markdown("### 📩 Admin Reply")
                        for _, reply in my_replies.tail(5).iterrows():
                            status = str(reply.get("status", ""))
                            message = str(reply.get("admin_reply", ""))
                            ...
                            st.caption(
                                ...
                                f"Updated: {reply.get('updated_at', '')}"
                            )
        except Exception as e:
            st.warning(f"Could not load admin replies: {e}")
            ...
            with st.form("dispute_correction_form"):
                ...
                if submit_dispute:
                    if not dispute_reason.strip():
                        ...
                        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                        csv_path = os.path.join(base_dir, "pending_corrections.csv")
                        headers = ["user_id","user_name","date","correction_type","requested_check_in","requested_check_out","reason","status","admin_reply","created_at","updated_at"]
...
# 5. GUEST DASHBOARD
elif st.session_state.current_page == "Guest Dashboard":
    col_left, col_mid, col_right = st.columns([1, 1.8, 1])
    with col_mid:
        st.markdown("<h2 style='text-align: center; color: #1F2937; font-family: \"Outfit\", sans-serif; font-weight: 800; margin-top: 1.5rem; margin-bottom: 1.5rem;'>⏳ Guest Dashboard - Account Status</h2>", unsafe_allow_html=True)
...
# 6. ADMIN DASHBOARD
elif st.session_state.current_page in ["Admin Dashboard", "Dashboard"]:
    # Guard: ensure authenticated with valid token before making admin API calls
    if not st.session_state.get("authenticated") or not getattr(api, "token", None):
        st.warning("⚠️ Session expired. Please log in again.")
        go_to_page("Login")
    ...
    try:
        ...
        try:
            ...
            st.warning(f"⚠️ Failed to load system statistics: {e}")
            
        tabs = ["👥 Participation", "🛡️ Admin Attendance", "📈 Attendance Overview", "📜 System Audit Logs",  "💬 Message"]
        if "admin_active_tab" not in st.session_state:
            st.session_state.admin_active_tab = "👥 Participation"
            
        selected_tab = st.radio("Console Tab", tabs, index=tabs.index(st.session_state.admin_active_tab), horizontal=True, label_visibility="collapsed")
        st.session_state.admin_active_tab = selected_tab
        
        if selected_tab == "👥 Participation":
            ...
            part_col1, part_col2, part_col3, part_col4, part_col5, part_col6 = st.columns(6)
            with part_col1:
                type_users = "primary" if st.session_state.active_part_view == "Total Users" else "secondary"
                if st.button(f"👥 Total User\n{stats.get('total_users', 0)}", key="btn_part_total_users", type=type_users, use_container_width=True):
                    st.session_state.active_part_view = "Total Users"
                    st.rerun()

            with part_col2:
                type_premium = "primary" if st.session_state.active_part_view == "Total Premium User" else "secondary"
                if st.button(f"💎 Total Premium User\n{stats.get('total_premium_users', 0)}", key="btn_part_total_premium", type=type_premium, use_container_width=True):
                    st.session_state.active_part_view = "Total Premium User"
                    st.rerun()

            with part_col3:
                type_developer = "primary" if st.session_state.active_part_view == "Total Developer" else "secondary"
                if st.button(f"💻 Total Developer\n{stats.get('total_developers', 0)}", key="btn_part_total_developer", type=type_developer, use_container_width=True):
                    st.session_state.active_part_view = "Total Developer"
                    st.rerun()

            with part_col4:
                type_admins = "primary" if st.session_state.active_part_view == "Total Admins" else "secondary"
                if st.button(f"🛡️ Total Admins\n{stats.get('total_admins', 0)}", key="btn_part_total_admins", type=type_admins, use_container_width=True):
            ...
            with part_col5:
                ...
                if st.button(f"⏳ Pending Approval\n{stats.get('pending_users', 0)}", key="btn_part_pending", type=type_pending, use_container_width=True):
            ...
            with part_col6:
                ...
                if st.button(f"✅ Approved Account\n{stats.get('approved_users', 0)}", key="btn_part_approved", type=type_approved, use_container_width=True):
            ...
            if st.session_state.active_part_view:
                ...
                if "admin_confirm_action" in st.session_state:
                    ...
                    if act['action'] == "delete":
                        st.warning(f"⚠️ Confirm action: **DELETE** user **{act['name']}** permanently?")
                    else:
                        target_label = act.get("target_role", act['action'].replace("make_", "").replace("convert_", "")).upper()
                        st.warning(f"⚠️ Confirm action: Change role of **{act['name']}** to **{target_label}**?")
                ...
                current_login_role = st.session_state.get("user_role", "").lower()
                if st.session_state.active_part_view == "Total Users":
                    display_list = [u for u in users_list if u["role"] == "User"]
                elif st.session_state.active_part_view == "Total Premium User":
                    display_list = [u for u in users_list if u["role"] == "Premium_User"]
                elif st.session_state.active_part_view == "Total Developer":
                    display_list = [u for u in users_list if u["role"] == "Developer"]
                elif st.session_state.active_part_view == "Total Admins":
                    if current_login_role == "admin":
                        display_list = [u for u in users_list if u["role"] == "Admin"]
                    else:
                        display_list = [u for u in users_list if u["role"] in ["Admin", "Super_Admin"]]
                    ...
                    for u in display_list:
                        if st.session_state.active_part_view == "Pending Approval":
                            ...
                            with st.expander(f"{u['name']} ({u['role']}) - Status: {u['approval_status']}"):
                                ...
                                # Role Management / Action operations
                                current_login_role = str(st.session_state.get("user_role", "")).lower()
                                if current_login_role in ["super_admin", "admin"]:
                                    st.markdown("---")
                                    st.markdown("##### ⚙️ Role Management")
                                    
                                    # Define options based on logged-in user
                                    if current_login_role == "super_admin":
                                        role_options = ["Super_Admin", "Admin", "Developer", "Premium_User", "User", "Guest"]
                                    else: # admin
                                        role_options = ["Developer", "Premium_User", "User", "Guest"]
                                    
                                    # Find index of current user's role if in options
                                    try:
                                        role_index = role_options.index(u['role'])
                                    except ValueError:
                                        role_index = 0
                                        
                                    col_sel_role, col_btn_role, col_btn_remove = st.columns([1.5, 1, 1])
                                    with col_sel_role:
                                        selected_target_role = st.selectbox(
                                            f"Select target role for {u['name']}",
                                            role_options,
                                            index=role_index,
                                            key=f"sel_role_{u['id']}",
                                            label_visibility="collapsed"
                                        )
                                    with col_btn_role:
                                        if st.button("Change Role", key=f"btn_change_role_{u['id']}", use_container_width=True):
                                            action_map = {
                                                "Super_Admin": "make_super_admin",
                                                "Admin": "make_admin",
                                                "Developer": "make_developer",
                                                "Premium_User": "make_premium_user",
                                                "User": "make_user",
                                                "Guest": "make_guest"
                                            }
                                            action_to_send = action_map[selected_target_role]
                                            ...
                                            st.rerun()
                                    with col_btn_remove:
                                        if st.button("❌ Remove User", key=f"btn_remove_{u['id']}", use_container_width=True):
                                            st.session_state.admin_confirm_action = {"id": u['id'], "name": u['name'], "action": "delete"}
                                            st.rerun()

        elif selected_tab == "💬 Message":
            st.markdown("### 💬 Attendance Correction Messages")
            ...
            if not os.path.exists(csv_path):
                st.info("No attendance correction messages found.")
            else:
                try:
                    ...
                    if df.empty:
                        ...
                        st.info(f"Pending correction requests: **{pending_count}**")
                        ...
                        for idx, row in df.iterrows():
                            ...
                            req_in = row.get("requested_check_in", "")
                            req_out = row.get("requested_check_out", "")
                            reason = row.get("reason", "")
                            created_at = row.get("created_at", "")

                            with st.container(border=True):
                                st.markdown(f"#### 👤 {user_name} | User ID: {user_id}")
                                st.write(f"**Date:** {req_date}")
                                st.write(f"**Correction Type:** {correction_type}")
                                st.write(f"**Requested Check-In:** {req_in}")
                                st.write(f"**Requested Check-Out:** {req_out}")
                                ...
                                if status.lower() == "pending":
                                    ...
                                    with col_accept:
                                        if st.button("✅ Accept", key=f"accept_corr_{idx}", use_container_width=True):
                                            df.at[idx, "status"] = "Accepted"
                                            df.at[idx, "admin_reply"] = "Your attendance correction request is accepted."
                                    ...
                                    with col_reject:
                                        if st.button("❌ Not Accept", key=f"reject_corr_{idx}", use_container_width=True):
                                            df.at[idx, "status"] = "Not Accepted"
                                            df.at[idx, "admin_reply"] = "Your attendance correction request is not acceptable."
                                            df.at[idx, "updated_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                            df.to_csv(csv_path, index=False)
                                            st.warning("Request marked as not accepted.")
                                            st.rerun()
                                else:
                                    st.info(row.get("admin_reply", ""))
            ...
            try:
                ...
                st.error(f"Failed to load admin attendance statistics: {e}")
                
        elif selected_tab == "📈 Attendance Overview":
            try:
...
<td style='padding: 10px; border: 1px solid #1F2937;'>{status_badge}</td>
                ...
                # Manual Attendance Override - Super Admin Only
                st.write("")
                ...
                if current_role == "super_admin":
                    ...
                    st.info("🔒 **Manual Attendance Override** is available only to Super Admin users.")
        ...
        elif selected_tab == "📜 System Audit Logs":
            st.markdown("### Admin Action Logs")
            
            st.markdown("#### 📜 Audit Logs")
            audit_logs_resp = api.get_audit_logs()
            if not audit_logs_resp or not audit_logs_resp.get("data"):
                st.info("No audit logs found.")
            else:
                audit_logs = audit_logs_resp["data"]
                # Render styled HTML Table to look premium (like the third image)
                html_table = """<table style='width: 100%; border-collapse: collapse; margin-top: 10px; color: #1F2937; font-family: "Outfit", "Inter", sans-serif; font-size: 0.9rem;'>
                <thead>
                <tr style='background-color: #2563EB; color: #FFFFFF; text-align: left; font-weight: bold;'>
                <th style='padding: 12px 10px; border: 1px solid #E2E8F0; width: 6%;'>id</th>
                <th style='padding: 12px 10px; border: 1px solid #E2E8F0; width: 18%;'>admin</th>
                <th style='padding: 12px 10px; border: 1px solid #E2E8F0; width: 44%;'>action</th>
                <th style='padding: 12px 10px; border: 1px solid #E2E8F0; width: 14%;'>target</th>
                <th style='padding: 12px 10px; border: 1px solid #E2E8F0; width: 18%;'>timestamp</th>
                </tr>
                </thead>
                <tbody>"""
                for item in audit_logs:
                    html_table += f"""<tr style='background-color: #FFFFFF; border-bottom: 1px solid #E2E8F0;'>
                    <td style='padding: 12px 10px; border: 1px solid #E2E8F0;'>{item['id']}</td>
                    <td style='padding: 12px 10px; border: 1px solid #E2E8F0; color: #1e3a8a; font-weight: 500;'>{item['admin']}</td>
                    <td style='padding: 12px 10px; border: 1px solid #E2E8F0; color: #334155;'>{item['action']}</td>
                    <td style='padding: 12px 10px; border: 1px solid #E2E8F0; color: #475569;'>• {item['target']}</td>
                    <td style='padding: 12px 10px; border: 1px solid #E2E8F0; color: #64748b;'>{item['timestamp']}</td>
                    </tr>"""
                html_table += "</tbody></table>"
                st.markdown(html_table, unsafe_allow_html=True)
                st.write("")
    ...
    st.markdown("<p style='color: #64748B;'>Configure shift timings and attendance grace periods.</p>", unsafe_allow_html=True)
    try:
        current_settings = api.get_attendance_settings()
        start_time = current_settings.get("start_time", "09:00")
        end_time = current_settings.get("end_time", "18:00")
        ...
        with st.form("timing_settings_form"):
            ...
            sc1, sc2, sc3 = st.columns(3)
            with sc1:
                new_start_h = st.selectbox("Hour", [f"{i:02d}" for i in range(1, 13)], index=int(start_h)-1, key="start_hour_sys")
            with sc2:
                new_start_m = st.selectbox("Minute", [f"{i:02d}" for i in range(60)], index=int(start_m), key="start_minute_sys")
            with sc3:
                new_start_p = st.selectbox("Period", ["AM", "PM"], index=0 if start_p == "AM" else 1, key="start_period_sys")
                
            st.markdown("**Office End Time**")
            ec1, ec2, ec3 = st.columns(3)
            with ec1:
                new_end_h = st.selectbox("Hour", [f"{i:02d}" for i in range(1, 13)], index=int(end_h)-1, key="end_hour_sys")
            with ec2:
                new_end_m = st.selectbox("Minute", [f"{i:02d}" for i in range(60)], index=int(end_m), key="end_minute_sys")
            with ec3:
                new_end_p = st.selectbox("Period", ["AM", "PM"], index=0 if end_p == "AM" else 1, key="end_period_sys")
            
            new_grace = st.number_input("Grace Period (Minutes)", min_value=0, max_value=480, value=grace_period, step=5, key="grace_period_input_sys")
...
# 7. USER PROFILE PAGE
    ...
    if not st.session_state.get("authenticated"):
        st.session_state.post_login_redirect = st.session_state.current_page
        go_to_page("Login")
    ...
    try:
        ...
        if st.session_state.get("editing_profile"):
            ...
            with st.container(border=True):
                ...
                with col_right:
                    ...
                    if profile_image_path:
                        resolved_path = resolve_image_path(profile_image_path)
                        if resolved_path and Path(resolved_path).exists():
                            st.image(resolved_path, use_container_width=True, caption="Approved face template")
                        else:
                            st.markdown("""
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            with st.container(border=True):
                st.markdown("### 👆 Biometrics — Fingerprint")
                st.caption("Register your device fingerprint (Windows Hello / Touch ID / Android) for touchless attendance.")

                # Query fingerprint status from backend
                try:
                    fp_status_data = api.webauthn_status(st.session_state.user_id)
                    has_fingerprint = fp_status_data.get("has_credential", False)
                except Exception:
                    has_fingerprint = False

                # Show fingerprint status badge
                if has_fingerprint:
                    st.markdown("""
                    ...
                    <div style='display:flex; align-items:center; gap:0.6rem; margin-bottom:1rem;'>
                        <span style='background:#FEF9C3; color:#854D0E; border:1px solid #FDE68A;
                                    border-radius:20px; padding:4px 14px; font-size:0.82rem; font-weight:700;'>
                            ⚠️ No Fingerprint Registered
                        </span>
                    </div>
                    """, unsafe_allow_html=True)

                # Handle return from WebAuthn registration flow (URL params)
                reg_result = st.query_params.get("fp_reg_result")
                if reg_result == "success":
                    st.success("✅ Fingerprint registered successfully! You can now use fingerprint attendance.")
                    st.query_params.clear()
                    st.rerun()
                elif reg_result == "error":
                    st.error("❌ Fingerprint registration failed. Please try again.")
                    st.query_params.clear()

                # Handle remove fingerprint action
                remove_result = st.query_params.get("fp_remove")
                if remove_result == "done":
                    st.success("✅ Fingerprint removed successfully.")
                    st.query_params.clear()
                    st.rerun()

                user_id_js = st.session_state.user_id
                backend_url_fp = api.base_url
                btn_label = "🔄 Update Fingerprint" if has_fingerprint else "👆 Register Fingerprint"
                token_val = api.token or ""
                remove_btn_html = ""
                if has_fingerprint:
                    remove_btn_html = f"""
                    <button onclick="removeFingerprint()"
                        style="background:#FEE2E2;color:#991B1B;border:1px solid #FECACA;
                               border-radius:24px;padding:0.65rem 1.4rem;font-size:0.9rem;
                               font-weight:700;cursor:pointer;display:inline-flex;
                               align-items:center;gap:0.4rem;">
                        🗑️ Remove Fingerprint
                    </button>"""

                # Use st.components.v1.html so <script> tags execute properly
                # st.markdown(unsafe_allow_html=True) strips scripts — components render in a real iframe
...
async function registerFingerprint() {{
    ...
    try {{
        ...
        setStatus('Waiting for your fingerprint / Windows Hello...', '#7C3AED');

        // 2. Browser biometric prompt
        const credential = await navigator.credentials.create({{
            publicKey: {{
                challenge: challengeBytes,
                rp: {{ name: 'Face AI Attendance'}},
                rp: {{ id: 'localhost' }},
                user: {{ id: userIdBytes, name: 'user_' + USER_ID, displayName: 'User ' + USER_ID }},
                pubKeyCredParams: [
        ...
        setStatus('Saving credential...', '#7C3AED');
        ...
        try {{
            ...
            pubKey = pkBytes ? b64url(pkBytes) : b64url(credential.response.attestationObject);
        }} catch(e) {{
            pubKey = b64url(credential.response.attestationObject);
        }}
        ...
        if (!regResp.ok) throw new Error('Backend save failed: ' + regResp.status);
        const regData = await regResp.json();

        if (regData.success) {{
            setStatus('✅ Fingerprint registered! Refreshing...', '#166534');
            const parentUrl = document.referrer || window.location.href;
            const targetUrl = parentUrl.split('?')[0] + '?fp_reg_result=success';
        ...
        if (e.name === 'NotAllowedError') {{
            setStatus('❌ Biometric cancelled or not allowed.', '#991B1B');
        }} else if (e.name === 'NotSupportedError') {{
            setStatus('❌ Your browser/device does not support WebAuthn.', '#991B1B');
        }} else {{
            setStatus('❌ Error: ' + e.message, '#991B1B');
        }}
...
async function removeFingerprint() {{
    if (!window.confirm('Remove your registered fingerprint? You can re-register anytime.')) return;
    setStatus('Removing...', '#854D0E');
    try {{
        const resp = await fetch(BACKEND + '/webauthn/credential/' + USER_ID, {{
            method: 'DELETE',
            headers: {{ 'Authorization': 'Bearer ' + TOKEN }}
        }});
        setStatus('✅ Removed. Refreshing...', '#166534');
        const parentUrl = document.referrer || window.location.href;
        const targetUrl = parentUrl.split('?')[0] + '?fp_remove=done';
        const a = document.createElement('a');
        a.href = targetUrl;
        a.target = '_parent';
        document.body.appendChild(a);
        ...
        setStatus('❌ Failed to remove. Try again.', '#991B1B');
    }}
}}
</script>
</body>
</html>"""
                st.components.v1.html(webauthn_component_html, height=120)
                ...
                if not cap.isOpened():
                    ...
                    if st.session_state.update_face_pose_idx >= 5:
                        ...
                        try:
                            ...
                            if st.session_state.user_role in ["Admin", "Super_Admin", "Developer"]:
                                st.session_state.current_page = "Admin Dashboard"
                            else:
                                go_to_page("User Dashboard")
                        except Exception as e:
