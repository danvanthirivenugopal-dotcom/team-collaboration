...
def _normalize_role(role: Any) -> str:
    """Normalize role names so User/user, Super Admin/Super_Admin all compare safely."""
    return str(role or "").strip().lower().replace(" ", "_").replace("-", "_")


def check_role(payload: Dict[str, Any], required_roles: list) -> bool:
    """Check if the token role matches any of the required roles, case-insensitively."""
    user_role = _normalize_role(payload.get("role"))
